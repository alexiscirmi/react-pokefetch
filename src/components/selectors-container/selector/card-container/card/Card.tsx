...
import { type TypesInterface, type DataInterface } from '../../../../../types'
...
export const Card: React.FC<DataInterface> = ({
  ...
  useEffect(() => {
    const typeCheck = async (): Promise<void> => {
      ...
      const type = typesData.types.find(
        (type: { type: string }) => type.type === types[0].type.name
      )
      setPokemonType(type)
    }
  ...
  return (
    <div
      ...
      style={pokemonType != null && { backgroundColor: pokemonType.color }}
    >
      {/* Header */}
      <div
